...
class Paratranz:
    ...
    async def download(self):
        os.makedirs(settings.filepath.tmp, exist_ok=True)
    ...
    async def _download_artifacts(self):
        ...
        with open(settings.filepath.tmp / f"paratranz_export.zip", "wb") as fp:
            fp.write(content)

    async def _extract_artifacts(self):
        with ZipFile(settings.filepath.tmp / f"paratranz_export.zip") as zfp:
